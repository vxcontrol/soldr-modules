...
local get_msi_version_name = "GetMsiVersion.ps1"
local data_osquery_msi_path = tostring(__tmpdir) .. "\\data\\binaries\\" .. osquery_msi_name
local get_msi_package_script_path = tostring(__tmpdir) .. "\\data\\scripts\\" .. get_msi_version_name
...
function Osquery:get_daemon_binary_path()
    ...
    if osquery_path ~= "" then return helpers.normalize_path(osquery_path) end
...
function Osquery:get_service_name()
    ...
    if osquery_service_name then return osquery_service_name end
...
function Osquery:is_installed()
    ...
    if osquery_service_name == "" then return false end
...
function Osquery:get_msi_version()
    local cmd = 'powershell -executionpolicy bypass -File ' .. get_msi_package_script_path .. ' ' .. data_osquery_msi_path
    local out = helpers.exec_cmd(cmd)
    __log.debugf("out of cmd '%s': %s", cmd, out)
...
function Osquery:uninstall()
    local cmd = "powershell -executionpolicy bypass -File " .. self:get_ps_manage_path() .. " -uninstall"

    local out = helpers.exec_cmd(cmd) or ""
    __log.debugf("out of cmd '%s': %s", cmd, out)

    local result = string.match(out, "System service 'osqueryd' uninstalled")

    if not result then return false, "error uninstalation osquery" end
