...
local function get_file_content(path)
    local cmd = "cat " .. path
    local content = sysinfo:exec_cmd(cmd) or ""
    --__log.debugf("out of cmd '%s': %s", cmd, content)
...
local function is_file_exist(path)
    local testcmd = 'test -f ' .. path .. [[ && echo "yes"]]
    local testout = sysinfo:exec_cmd(testcmd) or ""
    __log.debugf("out of cmd '%s': %s", testcmd, testout)
...
local function detect_package_manager()
    ...
    local out = sysinfo:exec_cmd(cmd)
    __log.debugf("out of cmd '%s': %s", cmd, out)
