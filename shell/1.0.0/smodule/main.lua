...
__api.add_cbs({
    data = function(src, data)
        ...
        local agent_id, _ = get_agent_id_by_dst(src, "any")
        local vxagent_id, _ = get_agent_id_by_dst(src, "VXAgent")
        local dst, _ = get_agent_src_by_id(agent_id, "VXAgent")

        __log.infof("receive data from '%s' vxagent_id '%s' agent_id '%s' dst '%s'", src, vxagent_id, agent_id, dst)
        ...
        if vxagent_id ~= "" then
            ...
            if type(retaddr) == "string" and retaddr ~= "" then
                ...
                __api.send_data_to(retaddr, cjson.encode(payload.out))
                return true
            end
        else
            -- send data to agent.
            ...
            local datap = cjson.encode(payload)
            __log.debugf("sending data '%s' to '%s'", dst, datap)
            __api.send_data_to(dst, datap)
        end
