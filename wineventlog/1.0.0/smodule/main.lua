...
local function send_logs(recn)
    ...
    if target_sock then
        local send = function(msg, len)
            if target_sock:send_data(msg) then
                ...
                __metric.add_int_counter("wel_server_events_send_count", cnt)
                __metric.add_int_counter("wel_server_events_send_size", size)
                ...
                local status, err = pcall(target_sock.close, target_sock)
                if not status then
                    __log.errorf("could not close target socket connection: '%s'", err)
                end
                target_sock = nil
            end
        end
    ...
    elseif log_lines_len >= limit_nums * 5 and now - last_sent > limit_secs then
        ...
        __metric.add_int_counter("wel_server_events_drop", #log_lines)
        log_lines = {}
        last_sent = now
    end
end
...
__api.add_cbs({
    data = function(src, data)
        ...
        if msg_data["type"] == "log" and type(msg_data["message"]) == "table" then
            __metric.add_int_counter("wel_server_events_recv_count", #msg_data["message"])
            __metric.add_int_counter("wel_server_events_recv_size", #data)
            if target_path ~= "" then
...
local function update_wel_stats()
    __metric.add_int_gauge_counter("wel_server_mem_usage", collectgarbage("count")*1024)
    __metric.add_int_gauge_counter("wel_server_events_queue_count", #log_lines)
end
